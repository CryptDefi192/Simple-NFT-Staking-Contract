...
// File openzeppelin-solidity/contracts/utils/math/SafeMath.sol@v4.4.1
...
// File openzeppelin-solidity/contracts/utils/structs/EnumerableSet.sol@v4.4.1
...
// File openzeppelin-solidity/contracts/security/Pausable.sol@v4.4.1
...
contract GHArtSpecialStaking is ReentrancyGuard, Pausable, Ownable, IERC721Receiver {
    ...
    /* ------------------------ NFT Staking ------------------------- */
    uint256 public START_DATETIME = 1648699200; //  (GMT): Thursday, March 31, 2022 4:00:00 AM
    uint256 public END_DATETIME = 1656561600; //  (GMT): Thursday, June 30, 2022 4:00:00 AM

    address public NFT_TOKEN_ADDRESS = address(0x832Ff53F75E44832Ea4928b788D82c7AF9A77310); // Special Collection
    address public REWARD_TOKEN_ADDRESS = address(0x2791Bca1f2de4661ED88A30C99A7a9449Aa84174); // USDC

    uint256 public TOKEN_REWARD_PER_DAY = 100000; // 0.1 USDC Per Day
    ...
    function setStakingDateTime (uint256 startDateTime, uint256 endDateTime) external onlyOwner {
    ...
    function setTokenRewardAddress(address _tokenAddress) external onlyOwner {
        REWARD_TOKEN_ADDRESS = _tokenAddress;
    }

    function setTokenRewardAmountPerDay(uint256 rewardPerDay) external onlyOwner {
        TOKEN_REWARD_PER_DAY = rewardPerDay;
    }
    ...
    function earned(address account) public view returns (uint256) {
        ...
        if (blockTime > END_DATETIME) blockTime = END_DATETIME;

        UserInfo memory user = userInfo[account];
        
        uint256 amount = 0;

        if (blockTime > user.lastUpdated) {
            amount = blockTime.sub(user.lastUpdated).mul(userBlanaces[account].length()).mul(TOKEN_REWARD_PER_DAY).div(1 days);
        }
    ...
    function totalStakedCount() public view returns (uint256) {
        ...
        for (uint256 i = 0; i < stakerList.length; i++) {
            totalCount += userBlanaces[stakerList[i]].length();
        }
        return totalCount;
    }
    ...
    function totalStakedMembers() public view returns (uint256) {
        ...
        for (uint256 i = 0; i < stakerList.length; i++) {
            if (userBlanaces[stakerList[i]].length() > 0) totalMembers += 1;
        }
        return totalMembers;
    }
    
    function stake( uint256[] calldata _tokenIDList) public nonReentrant whenNotPaused {
        require(block.timestamp >= START_DATETIME && block.timestamp <= END_DATETIME, "Staking is not activated");
        ...
        for (uint256 i = 0; i < _tokenIDList.length; i++) {
            ...
            userBlanaces[_msgSender()].add(_tokenIDList[i]);
        }
    }
    ...
    function unstake( uint256[] memory  tokenIdList) public nonReentrant {
        ...
        if (block.timestamp < END_DATETIME) {
            user.rewards = 0;
        }
    }

    function harvest() public nonReentrant {
        require(block.timestamp >= END_DATETIME, "Harvest is not activated");
